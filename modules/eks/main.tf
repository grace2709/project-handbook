...
resource "aws_eks_node_group" "default" {
  ...
  tags = { Name = "${var.project_name}-default-ng" }
}
...
resource "aws_eks_addon" "cloudwatch" {
  ...
  resolve_conflicts_on_create = "OVERWRITE"
  service_account_role_arn    = aws_iam_role.cloudwatch_addon.arn
  depends_on                  = [aws_eks_node_group.default]
}

# ── IAM: CloudWatch Addon Role ────────────────────────────────────────────────

resource "aws_iam_role" "cloudwatch_addon" {
  name = "${var.project_name}-cloudwatch-addon-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect = "Allow"
